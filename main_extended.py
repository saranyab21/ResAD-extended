...
def save_metrics_csv_and_plot(metrics_list, save_dir, setting, epoch):
    os.makedirs(save_dir, exist_ok=True)
    df = pd.DataFrame(metrics_list)
    csv_path = os.path.join(save_dir, f"{setting}_epoch{epoch}_metrics.csv")
    ...
    for key in value_keys:
        ...
        plt.title(f"{key} per Class - Epoch {epoch}")
        ...
        plt.savefig(os.path.join(save_dir, f"{setting}_epoch{epoch}_{key.replace(' ', '_').lower()}.png"))
        plt.close()
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            for class_name in CLASSES['unseen']:
                ...
                print(f"Epoch: {epoch}, Class Name: {class_name}, "
                      f"Image AUC | AP | F1: {img_auc:.3f} | {img_ap:.3f} | {img_f1_score:.3f}, "
                      f"Pixel AUC | AP | F1 | AUPRO: {pix_auc:.3f} | {pix_ap:.3f} | {pix_f1_score:.3f} | {pix_aupro:.3f}")
                ...
                metrics_list.append({
                    ...
                    "Image AUC": img_auc,
                    "Image AP": img_ap,
                    "Image F1": img_f1_score,
                    "Pixel AUC": pix_auc,
                    "Pixel AP": pix_ap,
                    "Pixel F1": pix_f1_score,
                    "Pixel AUPRO": pix_aupro
                })
        
            # Save epoch results as CSV + Plots
            save_metrics_csv_and_plot(metrics_list, save_dir=args.checkpoint_path, setting=args.setting, epoch=epoch)
            ...
            print('(Logps) Average Image AUC | AP | F1_Score: {:.3f} | {:.3f} | {:.3f}, '
                  'Average Pixel AUC | AP | F1_Score | AUPRO: {:.3f} | {:.3f} | {:.3f} | {:.3f}'.format(
                      img_auc1, img_ap1, img_f1_score1, pix_auc1, pix_ap1, pix_f1_score1, pix_aupro1))
            print('(BScores) Average Image AUC | AP | F1_Score: {:.3f} | {:.3f} | {:.3f}, '
                  'Average Pixel AUC | AP | F1_Score | AUPRO: {:.3f} | {:.3f} | {:.3f} | {:.3f}'.format(
                      img_auc2, img_ap2, img_f1_score2, pix_auc2, pix_ap2, pix_f1_score2, pix_aupro2))
            print('(Merged) Average Image AUC | AP | F1_Score: {:.3f} | {:.3f} | {:.3f}, '
                  'Average Pixel AUC | AP | F1_Score | AUPRO: {:.3f} | {:.3f} | {:.3f} | {:.3f}'.format(
            ...
            torch.save(checkpoint_latest, os.path.join(args.checkpoint_path, f'{args.setting}_latest.pth'))
